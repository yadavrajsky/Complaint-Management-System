    ...
    <div class="bg-white p-6 rounded-lg shadow-md mb-6">
        ...
        <form action="roles" method="post" class="space-y-4">
            <div>
                ...
                <input type="text" id="role" name="role" required class="mt-1 p-2 w-full border border-gray-300 rounded-md">
            </div>
            <div>
                <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-md">Add Role</button>
            </div>
        </form>
    </div>
<!-- Update Role Modal -->
<div id="updateRoleModal" class="fixed z-10 inset-0 overflow-y-auto hidden bg-black bg-opacity-50">
    <div class="flex items-center justify-center min-h-screen">
        <div class="relative bg-white w-1/3 p-8 rounded shadow-lg">
            ...
            <form action="roles" method="post" id="updateRoleForm" class="space-y-4">
                    <div>
                        <input type="hidden" name="roleId" id="updateRoleId">
                        <label for="updateRole" class="block text-sm font-medium text-gray-700">Role Name</label>
                        <input type="text" id="updateRole" name="role" required
                            class="mt-1 p-2 w-full border border-gray-300 rounded-md">
                    </div>
                    <div>
                        <button type="submit" name="action" value="edit" class="bg-blue-500 text-white px-4 py-2 rounded-md">Update Role</button>
                    </div>
            </form>
        </div>
    </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right">
                        ...
                        <button onclick="openUpdateModal('<%= role.getId() %>', '<%= role.getRole() %>')" class="text-blue-600 hover:text-blue-900">Edit</button>
                    </td>
                </tr>
                <% } %>
            </tbody>
